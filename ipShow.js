...
function show_msg(room_id, res, ini_flag, target, nowHeight) {
    ...
    for (var i = 0; i < res.length; i++) {
        if (res[i]["comment"] != undefined) {
            ...
            html += '<div class="r">';
            html += '<div class="comment_head"><span class="m_no">' + data["seq"] + '</span><span class="m_uname">' + name + '</span><span class="m_time">' + date_f(data.time) + '</span> <span>' + ipData + '</span><a>' + data.uid + '</div>';
